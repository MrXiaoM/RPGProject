...
public class GuiAddPower implements IGui {
    ...
    public Inventory createGui(Player player) {
        Inventory inv = Bukkit.createInventory(null, this.row * 9,
                RPGProject.i18n("gui.add-power.title").replace("%page%", String.valueOf(this.page))
    ...
    public Map<Integer, ItemStack> getGUIItems() {
        ...
        for (NamespacedKey power : PowerManager.getPowers().keySet()) {
            if (i >= j - 45 && i < j) {
                ...
                items.put(k,
                        Util.buildItem(Util.getPowerIconFromConfig(power),
        ...
        if (this.page - 1 > 0) {
            items.put(45, Util.buildItem(Enums.valueOf(Material.class,
        ...
        if (this.page < (PowerManager.getPowers().size() / 45.0D)) {
            items.put(53, Util.buildItem(Enums.valueOf(Material.class,
        ...
        items.put(49,
                Util.buildItem(
                        Enums.valueOf(Material.class, RPGProject.i18n("gui.add-power.items.back.material"),
                                Material.BARRIER),
    ...
    public void onClick(Player player, ItemStack clickedItem, int clickedSlot, InventoryView inv,
        ...
        if (clickedSlot < 45 && this.keys.containsKey(clickedSlot)) {
            ...
            this.close = true;
            this.remove = false;
            IGui gui = new GuiEditPower(player.getName(), this.rpg, power, true, 1);
            player.closeInventory();
