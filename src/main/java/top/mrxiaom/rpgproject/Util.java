...
public class Util {
    ...
    public static Material getPowerIconFromConfig(NamespacedKey power) {
        return Enums.valueOf(Material.class,
                RPGProject.i18n(
                                "power.materials." + power.getNamespace().toLowerCase() + "." + power.getKey().toLowerCase())
                        .toUpperCase(),
                Material.BOOK);
    }

    /**
     * RPGItems 全技能图标 TODO 添加到配置文件中
     *
     * @author MrXiaoM
     */
    @Deprecated
    public static Material getPowerIcon(Class<? extends Power> power) {
        ...
        if (power.equals(think.rpgitems.power.impl.Command.class)
                || power.equals(think.rpgitems.power.impl.CommandHit.class)) {
            return Material.COMMAND_BLOCK;
        }
        if (power.equals(think.rpgitems.power.impl.Consume.class)
                || power.equals(think.rpgitems.power.impl.ConsumeHit.class)) {
        ...
        if (power.equals(think.rpgitems.power.impl.PotionSelf.class)
                || power.equals(think.rpgitems.power.impl.PotionTick.class)) {
    ...
    public static ItemStack getEnchantedBook(Map<Enchantment, Integer> map) {
        ...
        for (Enchantment ench : map.keySet()) {
            ((EnchantmentStorageMeta) im).addStoredEnchant(ench, map.get(ench), true);
        }
        item.setItemMeta(im);
        return item;
    }
    ...
    public static ItemMeta getItemMeta(ItemStack item) {
        ...
        if (meta == null)
            return getMetaFormMaterial(item.getType());
        return meta;
    }
