...
public class GuiEditPower implements IGui{
	...
	boolean close = false;
	boolean remove = true;
	public GuiEditPower(String handlePlayer, RPGItem rpg, Power power, boolean add) {
	...
	public void onClick(Player player, ItemStack clickedItem, int clickedSlot, InventoryView inv,
		...
		if(clickedSlot < 45 && this.names.containsKey(clickedSlot)) {
			...
			if(cls.equals(int.class) || cls.equals(Integer.class)
				...
				if(shift) {
					if(left) {
					    PromptRegistry.registerPrompt(new PlayerPrompt(player, RPGProject.i18n("gui.edit-power.prompt.number")) {
					    	@Override
							public void run() {
					    		if(cls.equals(int.class) || cls.equals(Integer.class)) {
					    			try {
					    				int value = Integer.parseInt(this.getResponse());
					    				field.set(GuiEditPower.this.power, value);
					    			}catch(Throwable t) {
					    				RPGProject.send(player, RPGProject.i18n("gui.edit-power.prompt.number-1"));
					    			}
					    		} else if(cls.equals(double.class) || cls.equals(Double.class)) {
						    		try {
						    			double value = Double.parseDouble(this.getResponse());
						    			field.set(GuiEditPower.this.power, value);
						    		}catch(Throwable t) {
						    			RPGProject.send(player, RPGProject.i18n("gui.edit-power.prompt.number-1"));
						    		}
						    		...
						    		try {
						    			float value = Float.parseFloat(this.getResponse());
						    			field.set(GuiEditPower.this.power, value);
						    		}catch(Throwable t) {
						    			RPGProject.send(player, RPGProject.i18n("gui.edit-power.prompt.number-1"));
						    		}
			...
			// 类型: 文本
				...
				if(!shift && left && !right) {
					PromptRegistry.registerPrompt(new PlayerPrompt(player, RPGProject.i18n("gui.edit-power.prompt.string")) {
			    		@Override
						public void run() {
			    			try {
			    				field.set(GuiEditPower.this.power, this.getResponse());
			    			}catch(Throwable t) {
			...
			// 类型: 枚举
				...
				if(!shift) {
					if(left && !right) {
						...
						obj = (Enum<?>) cls.getEnumConstants()[obj.ordinal() + 1 < cls.getEnumConstants().length ? obj.ordinal() + 1 : 0];
						try {
					...
					if(right && !left) {
						PromptRegistry.registerPrompt(new PlayerPrompt(player, RPGProject.i18n("gui.edit-power.prompt.enum")) {
				    		@Override
							public void run() {
				    			Object obj = Enums.valueOfForce(cls, this.getResponse().toUpperCase(), null);
				    			if(obj != null) {
				    				try {
				    					field.set(GuiEditPower.this.power, obj);
				    				}catch(Throwable t) {
				    					t.printStackTrace();
				    					RPGProject.send(player, RPGProject.i18n("gui.edit-power.prompt.enum-1"));
				    				}
				    			}
				    			else {
			...
			// 类型: 药水效果
				...
				if(!shift) {
					...
					if(right && !left) {
						PromptRegistry.registerPrompt(new PlayerPrompt(player, RPGProject.i18n("gui.edit-power.prompt.potion")) {
				    		@Override
							public void run() {
				    			boolean flag = false;
				    			for(PotionEffectType effect : PotionEffectType.values()) {
				    				if(effect.getName().toUpperCase().equals(this.getResponse().toUpperCase())) {
				    					try {
					    					field.set(GuiEditPower.this.power, effect);
					    				}catch(Throwable t) {
					    					t.printStackTrace();
					    					RPGProject.send(player, RPGProject.i18n("gui.edit-power.prompt.potion-1"));
					    				}
				    					flag = true;
				    				}
				    			}
