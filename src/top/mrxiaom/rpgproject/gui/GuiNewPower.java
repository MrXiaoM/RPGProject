...
public class GuiNewPower implements IGui{
	String handlePlayer;
	RPGItem rpg;
	int row = 6;
	int page;
	boolean close = false;
	boolean remove = true;
	public GuiNewPower(String handlePlayer, RPGItem rpg, int page) {
	...
	public Map<Integer, ItemStack> getGUIItems(){
		...
		for(NamespacedKey power : PowerManager.getPowers().keySet()) {
			if(i >= j - 45 && i < j) {
				...
				items.put(k, Util.buildItem(Util.getPowerIcon(PowerManager.getPower(power)),
	...
	public void onClick(Player player, ItemStack clickedItem, int clickedSlot, InventoryView inv,
		...
		// 下一页
		if(clickedSlot == 43 && this.page + 1 < PowerManager.getPowers().size() / 45 + 1) {
			this.page++;
		}
		// 点击添加技能
